...
# I/O directories
from track_Bar import TrackBar
...
def part_1a():
    ...
    # shift_0 = cv2.imread(INPUT_DIR + 'test_lk3.png', 0) / 255.
    # shift_r2 = cv2.imread(INPUT_DIR + 'test_lk4.png', 0) / 255.
    ...
    # k = 0
    # shift_0 = cv2.GaussianBlur(shift_0, (k, k), sigma)
    # shift_r2 = cv2.GaussianBlur(shift_r2, (k, k), sigma)
    ...
    k_type = "normal"  # TODO: Select a kernel type
    sigma = 31 # TODO: Select a sigma value if you are using a gaussian kernel
    k = 5
    shift_0 = cv2.GaussianBlur(shift_0, (k, k), sigma)
    shift_r5_u5 = cv2.GaussianBlur(shift_r5_u5, (k, k), sigma)
...
def part_3a_1():
    ...
    k_size = 52 # TODO: Select a kernel size
...
def call_trackbar(src1, src2):
    trackBar = TrackBar(src1, src2)
    trackBar.start_track()
...
def part_4a():
    ...
    k_type = ""  # TODO: Select a kernel type
    sigma = 41  # TODO: Select a sigma value if you are using a gaussian kernel
    interpolation = cv2.INTER_CUBIC  # You may try different values
    border_mode = cv2.BORDER_REFLECT101  # You may try different values
    # call_trackbar(shift_0, shift_r20)

    u10, v10 = ps4.hierarchical_lk(shift_0, shift_r10, levels, k_size, k_type,
                                   sigma, interpolation, border_mode)

    u_v = quiver(u10, v10, scale=10, stride=10)
