...
PS_POS = os.path.join(INPUT_DIR, "ps6_pos")
PS_NEG = os.path.join(INPUT_DIR, "ps6_neg")
...
def visualize_mean_face(x_mean, size, new_dims):
    ...
    resized = np.resize(x_mean, size)
    normalized = normalize_and_scale(resized, new_dims)
    return normalized
...
def find_accuracy(actual, predict):
    matching = actual == predict
...
def part_2a():
    ...
    rand_accuracy = find_accuracy(ytrain, rand_y)
    ...
    # TODO: find which of these labels match ytrain and report its accuracy
    wk_accuracy = find_accuracy(ytrain, wk_results)
...
def part_4_a_b():
    ...
    vj_accuracy = find_accuracy(labels, predictions)
    ...
    vj_accuracy = find_accuracy(real_labels, predictions)
...
def part_4_c():
    ...
    neg = load_images_from_dir(NEG2_DIR)
    # pos = load_images_from_dir(PS_POS)
    # neg = load_images_from_dir(PS_NEG)
    ...
    persistent_obj.save(VJ)
    # VJ = persistent_obj.load()
